...
public class MyAdapter extends BaseAdapter {//listview를 만들기 위한 Adapter클래스
    private Context mContext;
    private int mResource;
    private ArrayList<MyItem> mItems = new ArrayList<MyItem>();

    public MyAdapter(Context context, int resource, ArrayList<MyItem> items) {  //생성자
        mContext = context;
        mItems = items;
        mResource = resource;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        TextView name = (TextView) convertView.findViewById(R.id.textItem1);  //item.xml에서 id가져오기
        // 어댑터가 관리하는 항목 데이터 중에서 position 위치의 항목의 문자열을 설정 텍스트뷰 객체에 설정
        name.setText(mItems.get(position).nName);  //메뉴이름 세팅

        // Set Text 02
        TextView age = (TextView) convertView.findViewById(R.id.textItem2);  //item.xml에서 가져오기
        age.setText(mItems.get(position).nAge);   //가격 세팅
        return convertView;
    }
}